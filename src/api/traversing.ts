...
export function parent<T extends Node>(
  this: Cheerio<T>,
  selector?: Parameters<typeof filter>[0]
): Cheerio<Element> {
  const set: Element[] = [];
...
export function parentsUntil<T extends Node>(
  ...
  filterBy?: Parameters<typeof filter>[0]
): Cheerio<Element> {
  const parentNodes: Element[] = [];
...
export function nextUntil<T extends Node>(
  ...
  filterSelector?: Parameters<typeof filter>[0]
): Cheerio<Element> {
  const elems: Element[] = [];
  let untilNode: Node | undefined;
  let untilNodes: Node[] | undefined;
...
export function filter(
  ...
  match: string | FilterFunction<Element> | Node | Cheerio<Node>,
  container = this
): Cheerio<Element> {
...
export function not<T extends Node>(
  this: Cheerio<T> | T[],
  match: T | Cheerio<T> | string | FilterFunction<T>,
  container?: Cheerio<T>
): Cheerio<T> {
  const instance = container ?? this;

  if (!isCheerio(instance)) {
    throw new Error('Not able to create a Cheerio instance.');
  }
  ...
  if (typeof match === 'string') {
    ...
    const matches = new Set<Node>(
      select.filter(match, elements, instance.options)
    );
...
function traverseParents<T extends Node>(
  self: Cheerio<T>,
  elem: Node | null,
  selector: Parameters<typeof filter>[0] | undefined,
  limit: number
): Node[] {
  const elems: Node[] = [];
